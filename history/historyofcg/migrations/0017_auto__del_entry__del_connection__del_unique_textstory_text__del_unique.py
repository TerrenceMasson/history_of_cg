...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'ImageStory', fields ['image']
        db.delete_unique('historyofcg_imagestory', ['image'])

        # Removing unique constraint on 'VideoStory', fields ['video']
        db.delete_unique('historyofcg_videostory', ['video'])

        # Removing unique constraint on 'TextStory', fields ['text']
        db.delete_unique('historyofcg_textstory', ['text'])
    ...
    def backwards(self, orm):
        ...
        db.create_table('historyofcg_connection', (
            ('image', self.gf('django.db.models.fields.URLField')(max_length=200, unique=True)),
            ('entry_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['historyofcg.Entry'], unique=True, primary_key=True)),
        ))
        db.send_create_signal('historyofcg', ['Connection'])

        # Adding unique constraint on 'TextStory', fields ['text']
        db.create_unique('historyofcg_textstory', ['text'])

        # Adding unique constraint on 'VideoStory', fields ['video']
        db.create_unique('historyofcg_videostory', ['video'])

        # Adding unique constraint on 'ImageStory', fields ['image']
        db.create_unique('historyofcg_imagestory', ['image'])
